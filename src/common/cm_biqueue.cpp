...
biqueue_t* biqueue_init(uint32 node_size, uint32 count)
{
    ...
    if (sizeof(biqueue_node_t) >= node_size) {
        LOG_PRINT_ERROR("biqueue_init: invalid node_size(%d)", node_size);
        return NULL;
    }
...
retry_get_node:
    ...
    if(node != NULL) {
        if (node->magic != 0) {
            LOG_PRINT_ERROR("biqueue_alloc: queue %p node %p node_id %d invalid magic", queue, node, node->node_id);
        }
...
biqueue_node_t* biqueue_get_node(biqueue_t *queue, uint32 node_id)
{
    ...
    if (slot_index >= queue->slot_index) {
        LOG_PRINT_ERROR("biqueue_get_node: invalid node_id(%d), slot_index %d", node_id, queue->slot_index);
        return NULL;
    }
    node = (biqueue_node_t *)(queue->slot[slot_index] + index * queue->node_size);
    if (node->magic != BIQUEUE_MAGIC) {
        LOG_PRINT_ERROR("biqueue_get_node: queue %p node %p node_id %d invalid magic", queue, node, node->node_id);
    }
    return node;
}
...
bool32 biqueue_free(biqueue_t *queue, biqueue_node_t* node)
{
    biqueue_lock(queue);
    if (node->magic != BIQUEUE_MAGIC) {
        LOG_PRINT_ERROR("biqueue_free: queue %p node %p node_id %d invalid magic", queue, node, node->node_id);
    }
    node->magic = 0;
...
bool32 biqueue_free_to_tail(biqueue_t *queue, biqueue_node_t* node)
{
    biqueue_lock(queue);
    if (node->magic != BIQUEUE_MAGIC) {
        LOG_PRINT_ERROR("biqueue_free_to_tail: node %p node_id %d invalid magic", node, node->node_id);
    }
    node->magic = 0;
    UT_LIST_ADD_LAST(list_node, queue->free_list, node);
    biqueue_unlock(queue);
