...
#define M_CONTEXT_PAGE_SIZE       (1024 * 8)
...
{
    memory_area_t *area = (memory_area_t *)malloc((size_t)(ut_align8(sizeof(memory_area_t)) + size));
    if (area) {
        area->size = size;
        ...
        area->buf = (char *)area + ut_align8(sizeof(memory_area_t));
    }
    ...
    if (area->is_extend) {
        for (uint32 i = 0; i < MEM_AREA_PAGE_ARRAY_SIZE; i++) {
            ...
            while (page) {
                ...
                if ((char *)page < (char *)area ||
                    (char *)page > (char *)area + ut_align8(sizeof(memory_area_t)) + area->size) {
                    free(page);
                }
                page = UT_LIST_GET_FIRST(area->free_pages[i]);
            }
...
{
    uint32 n = ut_2_log(page_size);

    ut_a(n < MEM_AREA_PAGE_ARRAY_SIZE);

    spin_lock(&area->lock, NULL);
    UT_LIST_ADD_LAST(list_node, area->free_pages[1], page);
    spin_unlock(&area->lock);
}
...
memory_pool_t* mpool_create(memory_area_t *area, uint32 local_page_count, uint32 max_page_count, uint32 page_size)
{
    ...
    memory_pool_t *pool = (memory_pool_t *)malloc(ut_align8(sizeof(memory_pool_t)));
    if (pool != NULL) {
        pool->lock = 0;
