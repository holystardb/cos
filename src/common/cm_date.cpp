...
bool32 cm_str2week(text_t *value, uint8 *week)
{
    ...
    for (uint8 i = 0; i < day_number_of_week; i++) {
        if (cm_text_str_contain_ncase_equal(value, g_week_days[i].str, 3)) {
            *week = i;
            return TRUE;
        }
    }
...
static inline status_t cm_fetch_format_item(text_t *fmt, format_item_t **item, text_t *extra, bool32 do_trim)
{
    ...
    for (i = 0; i < DATE_FORMAT_COUNT; i++) {
        cmp_text.len = ut_min(g_formats[i].name.len, fmt->len);

        if (cm_text_ncase_equal(&g_formats[i].name, &cmp_text)) {
            *item = &g_formats[i];
                                           ...
                                           uint32 prec, text_t *date_text, uint32 max_len)
{
    ...
    switch (item->id) {
        ...
        case FMT_YEAR3:
        case FMT_YEAR4:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%04u", detail->year);
        ...
        case FMT_CENTURY:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", cm_get_century(detail));
            break;

        case FMT_DAY_OF_WEEK:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%d", detail_ex->day_of_week + 1);
            break;
        ...
        case FMT_HOUR_OF_DAY12:
            ...
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", hh12_value);
            break;

        case FMT_HOUR_OF_DAY24:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02d", detail->hour);
            break;

        case FMT_QUARTER:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%u", detail_ex->quarter);
            break;

        case FMT_SECOND:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", detail->sec);
            break;

        case FMT_SECOND_PASS:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%05u", detail_ex->seconds);
            break;

        case FMT_WEEK_OF_YEAR:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", detail_ex->week);
            break;

        case FMT_WEEK_OF_MONTH:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%d", (detail->day / 7) + 1);
            break;

        case FMT_DAY_OF_MONTH:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", (detail->day));
            break;

        case FMT_DAY_OF_YEAR:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%03u", detail_ex->day_of_year);
            break;

        case FMT_FRAC_SECOND1:
        case FMT_FRAC_SECOND2:
        case FMT_FRAC_SECOND3:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%03u", detail->millisec);
        ...
        case FMT_FRAC_SECOND6:
            ...
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%06u", frac);
        ...
        case FMT_MINUTE:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", detail->min);
            break;

        case FMT_MONTH:
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02u", detail->mon);
            break;

        case FMT_TZ_HOUR:
            tz_hour = TIMEZONE_GET_HOUR(detail->tz_offset);
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%+03d", tz_hour);
            break;

        case FMT_TZ_MINUTE:
            tz_minute = TIMEZONE_GET_MINUTE(detail->tz_offset);
            snprintf(item_str, FORMAT_ITEM_BUFFER_SIZE, "%02d", tz_minute);
            break;
                               ...
                               uint32 max_len)
{
    ...
    while (fmt->len > 0) {
        if (cm_fetch_format_item(fmt, &item, &fmt_extra, FALSE) != CM_SUCCESS) {
            return CM_ERROR;
        }
        ...
        CM_RETURN_IF_ERROR(cm_append_date_text(detail, &detail_ex, item,
...
status_t cm_time2text(time_t time, text_t *fmt, text_t *text, uint32 max_len)
{
    ...
    return cm_detail2text(&detail, &format_text, CM_MAX_DATETIME_PRECISION, text, max_len);
}
...
static status_t cm_get_month_by_name(text_t *date_text, uint32 *mask, uint8 *mon)
{
    ...
    *mask |= MASK_MONTH;
    ...
    for (uint32 i = 0; i < 12; i++) {  // have 12 month
        ...
        if (!cm_text_ncase_equal(&cmp_text, &g_month_names[i])) {
            continue;
        }
...
static status_t cm_get_month_by_abbr_name(text_t *date_text, uint32 *mask, uint8 *mon)
{
    ...
    *mask |= MASK_MONTH;
    ...
    for (uint32 i = 0; i < 12; i++) {
        ...
        if (!cm_text_ncase_equal(&cmp_text, &mon_text)) {
            continue;
        }
...
static status_t cm_get_month_by_roman_name(text_t *date_text, uint32 *mask, uint8 *mon)
{
    ...
    *mask |= MASK_MONTH;
    ...
    for (int32 i = 11; i >= 0; i--) {
        ...
        if (!cm_text_ncase_equal(&cmp_text, &g_month_roman_names[i])) {
            continue;
        }
...
static uint32 cm_get_num_len_in_str(const text_t *text, uint32 part_len, bool32 with_sign)
{
    ...
    for (i = 0; i < ut_min(part_len, text->len); i++) {
                                        ...
                                        uint32 *num_value, uint32 *act_len_val)
{
    ...
    *act_len_val = act_len;
    ...
    if (act_len != 0) {
        MEMS_RETURN_IFERR(memcpy_s(num, sizeof(num), num_text->str, (size_t)act_len));
    }
                                      ...
                                      uint32 *part_value)
{
    ...
    if (cm_verify_number(date_text, part_len, start_value, end_value, part_value, &act_len) != CM_SUCCESS) {
        return CM_ERROR;
    }
                                        ...
                                        uint32 *mask)
{
    ...
    switch (fmt_item->id) {
        ...
        case FMT_SLASH:
            part_text.len = 1;
            if (!cm_text_ncase_equal(&part_text, &fmt_item->name)) {
                return CM_ERROR;
            }
        ...
        case FMT_DQ_TEXT:
            ...
            if (!cm_text_ncase_equal(&part_text, fmt_extra)) {
                return CM_ERROR;
            }
        ...
        case FMT_FRAC_SEC_VAR_LEN:
            ...
            if (fmt_item->placer <= 0) {
                //CM_THROW_ERROR_EX(ERR_ASSERT_ERROR, "fmt_item->placer(%d) > 0", fmt_item->placer);
                return CM_ERROR;
            }
...
static status_t cm_text2date_detail(const text_t *text, const text_t *fmt, date_detail_t *datetime, uint32 datetype)
{
    ...
    while (fmt_text.len > 0) {
        if (cm_fetch_format_item(&fmt_text, &fmt_item, &fmt_extra, TRUE) != CM_SUCCESS) {
            return CM_ERROR;
        }
...
status_t cm_text2timestamp_tz(const text_t *text, const text_t *fmt, timezone_info_t default_tz, timestamp_tz_t *tstz)
{
    ...
    //if (cm_text2date_detail(text, &fmt_text, &detail, CT_TYPE_TIMESTAMP_TZ) != CM_SUCCESS) {
    //    CM_SET_DATETIME_FMT_ERROR;
    //    return CM_ERROR;
    //}
    ...
    //    CM_THROW_ERROR(ERR_TYPE_OVERFLOW, "TIMESTAMP_TZ");
    //    return CM_ERROR;
    //}
...
status_t cm_text2date(const text_t *text, const text_t *fmt, date_t *date)
{
    ...
    cm_text2date_init(&detail);
    //if (cm_text2date_detail(text, &fmt_text, &detail, CT_TYPE_DATE) != CM_SUCCESS) {
    //    CM_SET_DATETIME_FMT_ERROR;
    //    return CM_ERROR;
    //}
...
status_t cm_str2time(char *date, const text_t *fmt, time_t *time_stamp)
{
    ...
    CM_RETURN_IF_ERROR(cm_text2date(&date_text, fmt, &date_stamp));
...
status_t cm_text2date_fixed(const text_t *text, const text_t *fmt, date_t *date)
{
    ...
    //if (cm_text2date_detail(text, fmt, &detail, CT_TYPE_DATE) != CM_SUCCESS) {
    //    CM_SET_DATETIME_FMT_ERROR;
    //    return CM_ERROR;
    //}
...
status_t cm_fetch_date_field(text_t *text, uint32 minval, uint32 maxval, char spilt_char, uint32 *field_val)
{
    ...
    cm_trim_text(text);
    num_len = cm_get_num_len_in_str(text, text->len, FALSE);
    CM_RETURN_VALUE_IF_TRUE(num_len == 0, CM_ERROR);

    CM_RETURN_IF_ERROR(cm_check_number(text, num_len, minval, maxval, (int64*)field_val));
    text->str += num_len;
    text->len -= num_len;
...
status_t cm_text2date_def(const text_t *text, date_t *date)
{
    ...
    do {
        // 1-1-1 ~ 1990-01-01
        CM_BREAK_IF_TRUE((date_text.len < 5));

        // year
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, CM_MIN_YEAR, CM_MAX_YEAR, '-', &field_val));
        detail.year = (uint16)field_val;

        // month
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 1, 12, '-', &field_val));
        detail.mon = (uint8)field_val;

        // day
        mon_days = (uint32)g_month_days[IS_LEAP_YEAR(detail.year)][detail.mon - 1];
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 1, mon_days, '\0', &field_val));
        detail.day = (uint8)field_val;

        ret = (date_text.len == 0) ? CM_SUCCESS : CM_ERROR;
    } while (0);
...
status_t cm_text2timestamp_def(const text_t *text, date_t *date)
{
    ...
    do {
        ...
        CM_BREAK_IF_TRUE((date_text.len < 11));
        ...
        // day
        mon_days = (uint32)g_month_days[IS_LEAP_YEAR(detail.year)][detail.mon - 1];
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 1, mon_days, ' ', &field_val));
        detail.day = (uint8)field_val;

        // hour
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 0, 23, ':', &field_val));
        detail.hour = (int16)field_val;

        // minute
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 0, 59, ':', &field_val));
        detail.min = (uint8)field_val;

        // second
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 0, 59, '\0', &field_val));
        ...
        CM_BREAK_IF_TRUE(CM_TEXT_BEGIN(&date_text) != '.');
        CM_REMOVE_FIRST(&date_text);
        cm_ltrim_text(&date_text);
        CM_BREAK_IF_TRUE((date_text.len == 0 || date_text.len > 6));  // max precision is 6
        // append '0' for frac second
        //CM_RETURN_IF_ERROR(cm_text2str(&date_text, buf, sizeof(buf)));
        date_text.str = buf;
        date_text.len = (uint32)strlen(buf);
        while (date_text.len < 6) {
            CM_TEXT_APPEND(&date_text, '0');
        }
        CM_BREAK_IF_ERROR(cm_fetch_date_field(&date_text, 0, 999999, '\0', &field_val));
...
status_t cm_round_date(date_t date, text_t *fmt, date_t *result)
{
    ...
    bool32 need_round = FALSE;

    cm_decode_date(date, &detail);
    cm_get_detail_ex(&detail, &detail_ex);
    CM_RETURN_IF_ERROR(cm_fetch_format_item(fmt, &item, &fmt_extra, FALSE));
...
static int64 cm_cnvrt_datetime_from_binary_to_int(const uchar *ptr)
{
    ...
    int32 fracpart = cm_ptr3_to_sint_little_endian(ptr + CM_DATETIME_PRECISION_5);
...
status_t cm_text2date_mysql(const text_t *text, const text_t *fmt, date_t *date, uint32 datatype)
{
    ...
    ////cm_text2date_init(&detail);
    ////if (cm_text2date_detail(text, &fmt_text, &detail, datatype) != CM_SUCCESS) {
    ////    CM_SET_DATETIME_FMT_ERROR;
    ////    return CM_ERROR;
    ////}
...
{
    date_t res;
    if (cm_text2date_mysql(text, fmt, &res, datatype) != CM_SUCCESS) {
        return CM_ERROR;
    }
    //switch (datatype) {
    //    case CT_TYPE_DATETIME_MYSQL:
