...
static void srv_close_socket(connection_t* conn, Vio* vio)
{
    if (vio->sock_fd != INVALID_SOCKET) {
        LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "srv_close_socket: connection(%d : %d)", conn->id, vio->sock_fd);
...
static bool32 srv_epoll_accept(reactor_t* reactor, my_socket listen_fd, my_socket accept_fd)
{
    ...
    if (conn == NULL) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "epoll_accept: error for alloc conn, accept fd %d", accept_fd);
        return FALSE;
    }
    
    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "epoll_accept: connection(id=%d, fd=%d) added event to epoll", conn->id, accept_fd);
    ...
    if (g_socks_mgr.type == SOCKS_PROXY || g_socks_mgr.type == L_PROXY) {
        ...
        if (-1 == reactor_add_read(conn->reactor, accept_fd, &epoll_data, g_socks_mgr.poll_timeout_sec)) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "epoll_accept: connection(id=%d, fd=%d) can not add event to epoll", conn->id, accept_fd);
            srv_free_conn(conn);
            return FALSE;
        }
...
static bool32 socks_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_req: timeout for client, connection(%d : %d)",
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_req: connection(%d : %d) is closed by client.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_req: error for read from client. connection(%d : %d), error %d",
    ...
    if (VIO_SUCCESS != ret)
    {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_req: error for send request to server, connection(%d : %d), error %d",
...
static bool32 socks_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_rsp: connection(%d : %d) is closed by server.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_rsp: error for read from server. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static bool32 do_check_connection_status(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "do_check_connection_status: timeout for connect server(%s:%d), connection(%d : %d)",
    ...
    if (ret != VIO_SUCCESS) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "do_check_connection_status: error for connect server(%s:%d), connection(%d : %d), error %d",
            host, port, conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        return FALSE;
    }

    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "do_check_connection_status: connected to server(%s:%d), connection(%d : %d)",
        host, port, conn->id, conn->server_vio.sock_fd);
...
static void socks_async_connect_server(connection_t *conn)
{
    ...
    if (g_socks_mgr.type == SOCKS_PROXY) {
        if (conn->atype == HOST_IPV4) {
            ...
            {
                LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_async_connect_server: error for connect server(%s:%d), connection(%d), error %d",
            ...
            //getaddrinfo()
            if (phost == NULL) {
                LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_async_connect_server: error for convert domain(%s:%d), connection(%d)",
        ...
        if (VIO_SUCCESS != ret)
        {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_async_connect_server: error for connect rproxy(%s:%d), connection(%d), error %d",
                g_socks_mgr.remote_host, g_socks_mgr.remote_port, conn->id, conn->server_vio.error_no);
            goto err_exit;
        }
    }
    
    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "socks_async_connect_server: connecting to server(%s:%d), connection(%d : %d)",
...
static void socks_handle_conn(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_conn: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_conn: error for read from client, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    if (conn->buf[0] != 0x05 || conn->buf[1] == 0) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_conn: invalid request of socks, connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (is_need_auth) {
        ...
        if (is_allowed_auth == FALSE) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_conn: invalid authentication, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
            goto err_exit;
        }
        conn->buf[1] = 0x02;
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_conn: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    //
...
static void socks_handle_auth(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_auth: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_auth: error for read from client, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    // user
    if (conn->buf[1] == 0) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_auth: invalid request of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    user_len = conn->buf[1];
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_auth: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    // 
...
static void socks_handle_host(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: error for read from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    if (conn->buf[0] != 0x05) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: invalid request of socks. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    ...
    if (cmd != 0x01) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: UDP is not supported. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    ...
    case HOST_IPV4: // ipv4
    {
        ...
        ret = vio_read(&conn->client_vio, conn->buf, conn->buf_len);
        if (VIO_SUCCESS != ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: error for read domain from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
            goto err_exit;
        }
    ...
    case HOST_IPV6:
        ...
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: addr/ipv6 is not supported. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret)
    {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "socks_handle_host: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    // 
...
static bool32 lproxy_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_req: timeout for client, connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_req: connection(%d : %d) is closed by client.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_req: error for read from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (conn->is_first_content) {
        ...
        pos += 2;
        LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_req: server(%s:%d)", conn->host, conn->port);
    }
    ...
    if (VIO_SUCCESS != ret)
    {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_req: error for send request to rproxy, connection(%d : %d), error %d",
            conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        goto err_exit;
    }
...
static bool32 lproxy_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_rsp: connection(%d : %d) is closed by rproxy.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_rsp: error for read from rproxy. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_rsp: connection(%d : %d) is closed by rproxy.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_rsp: error for read from rproxy. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "lproxy_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static bool32 rproxy_parse_contents_req(connection_t *conn, bool32 *is_first_content)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: connection(%d : %d) is closed by lproxy.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: error for read from lproxy. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: connection(%d : %d) is closed by lproxy.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: error for read from lproxy. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
    ...
    *is_first_content = mach_read_from_1((unsigned char*)conn->buf);
    ...
    if (memcmp(g_socks_mgr.auth_md5_buf, conn->buf + pos, MD5_BLOCK_SIZE) != 0) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: invalid user or password. connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    pos += MD5_BLOCK_SIZE;
    if (*is_first_content != 0) {
        ...
        pos += 2;
        LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: connection(%d : %d) SERVER(%s : %d)",
            conn->id, conn->client_vio.sock_fd, conn->host, conn->port);
    }
    
    conn->offset = pos;
    
    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "rproxy_parse_contents_req: connection(%d : %d : %d) SERVER(%s : %d)",
...
static bool32 rproxy_async_connect_server(connection_t *conn)
{
    ...
    phost = gethostbyname(conn->host);
    if (phost == NULL) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_async_connect_server: error for convert domain, connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    memset(&sin, 0, sizeof(struct sockaddr_in));
    ...
    if (VIO_SUCCESS != ret)
    {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_async_connect_server: error for connect server, connection(%d), error %d",
            conn->id, conn->server_vio.error_no);
        goto err_exit;
    }

    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "rproxy_async_connect_server: connecting to server(%s:%d), connection(%d : %d)",
        conn->host, conn->port, conn->id, conn->server_vio.sock_fd);
...
static bool32 rproxy_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    else
    {
        ...
        if (VIO_SUCCESS != ret)
        {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_handle_contents_req: error for send request to server, connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
            goto err_exit;
        }
...
static bool32 rproxy_check_connection_status(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret)
    {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_check_connection_status: error for send request to server, connection(%d : %d), error %d",
            conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        goto err_exit;
    }
...
static bool32 rproxy_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_handle_contents_rsp: timeout for server, connection(%d : %d)",
            conn->id, conn->server_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "rproxy_handle_contents_rsp: connection(%d : %d) is closed by server.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_handle_contents_rsp: error for read from server. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "rproxy_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static void socks_epoll_events(my_socket fd, struct epoll_event* event)
{
    ...
    LOGGER_DEBUG(LOGGER, LOG_MODULE_COMMON, "socks_epoll_events: connection(%d : %d : %d), fd %d status %d",
...
static void print_statistics()
{
    ...
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "-----------------------------------------------------------------------");
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "| connection count |   total % 8d     |   inprocess % 4d          |", alloc_count, inprocess_count);
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "-----------------------------------------------------------------------");
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "| thread count     |   free  % 8d     |   working   % 4d          |", free_threads, working_threads);
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "-----------------------------------------------------------------------");
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "| client request   |   count % 8d     |   tps       % 4d          |", req_count, tps);
    LOGGER_ERROR(LOGGER, LOG_MODULE_COMMON, "-----------------------------------------------------------------------");
}
...
int main1(int argc, char *argv[])
{
    ...
    LOGGER_INFO(LOGGER, LOG_MODULE_COMMON, "service is started. host %s:%d", g_socks_mgr.local_host, g_socks_mgr.local_port);
