...
void aes_encrypt_file(char *src_file, char *output_file)
{
    ...
    while (len > 0) {
        ...
        aes_encrypt(&aes, (uint8*)buf, (int)len, out, 1024*1024);
...
void aes_encrypt_file_block(char *src_file, long long begin_pos, long long end_pos, char *output_file)
{
    ...
    while (1) {
        ...
        aes_encrypt(&aes, (uint8*)buf, (uint32)len, out, 1024*1024);
        fwrite(out, 1, len, w_fp);
        fflush(w_fp);
...
void aes_decrypt_file(char *src_file, char *output_file)
{
    ...
    while (len > 0) {
        ...
        aes_decrypt(&aes, (uint8*)buf, (int)len, out, 1024 * 1024);
...
int main44(int argc, char *argv[])
{
    ...
    uint32 type, include_file_count = 0, suffix_count = 0, dir_count = 0;
    ...
    if (thread_count > 1) {
        for (int i = 0; i < thread_count && i < M_THREAD_COUNT; i++) {
            ...
            if (!os_is_valid_thread(g_file_mgr.handle[i])) {
