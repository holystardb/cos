...
void aes_cbc()
{
    ...
    aes_set_key(&aes, key, (uint32)strlen(key), AES_CYPHER_128, iv);
    aes_padding_buf(in, 19, 256);
    printf("%s\n", in);
    len = aes_get_cypher_len((int)strlen(in));
    aes_encrypt_cbc(&aes, (uint8*)in, len, out1, len);
    memset(out2, 0x00, 256);
    aes_decrypt_cbc(&aes, (uint8*)out1, len, out2, len);
    printf("%s\n", out2);
}
...
void aes()
{
    ...
    printf("%s\n", in);
    aes_encrypt(&aes, (uint8*)in, (uint32)strlen(in), out1, 256);
    memset(out2, 0x00, 256);
    aes_decrypt(&aes, (uint8*)out1, aes_get_cypher_len((uint32)strlen(in)), out2, 256);
    printf("%s\n", out2);
}
