#include "os_type.h"
#include "os_biqueue.h"
#include "os_thread.h"
#include "os_thread_pool.h"
#include "os_thread_group.h"
#include "os_vio_socket.h"
#include "os_epoll.h"
#include "os_epoll_reactor.h"
#include "os_log.h"
#include "os_config.h"
#include "os_getopt.h"
#include "os_file.h"
...
static void srv_close_socket(connection_t* conn, Vio* vio)
{
    if (vio->sock_fd != INVALID_SOCKET) {
        LOG_PRINT(LOG_DEBUG, "srv_close_socket: connection(%d : %d)", conn->id, vio->sock_fd);
...
static bool32 srv_epoll_accept(reactor_t* reactor, my_socket listen_fd, my_socket accept_fd)
{
    ...
    if (conn == NULL) {
        LOG_PRINT(LOG_ERROR, "epoll_accept: error for alloc conn, accept fd %d", accept_fd);
        return FALSE;
    }
    
    LOG_PRINT(LOG_DEBUG, "epoll_accept: connection(id=%d, fd=%d) added event to epoll", conn->id, accept_fd);
    ...
    if (g_socks_mgr.type == SOCKS_PROXY || g_socks_mgr.type == L_PROXY) {
        ...
        if (-1 == reactor_add_read(conn->reactor, accept_fd, &epoll_data, g_socks_mgr.poll_timeout_sec)) {
            LOG_PRINT(LOG_NOTICE, "epoll_accept: connection(id=%d, fd=%d) can not add event to epoll", conn->id, accept_fd);
            srv_free_conn(conn);
            return FALSE;
        }
...
static bool32 socks_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_contents_req: timeout for client, connection(%d : %d)",
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "socks_handle_contents_req: connection(%d : %d) is closed by client.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "socks_handle_contents_req: error for read from client. connection(%d : %d), error %d",
    ...
    if (VIO_SUCCESS != ret)
    {
        LOG_PRINT(LOG_NOTICE, "socks_handle_contents_req: error for send request to server, connection(%d : %d), error %d",
...
static bool32 socks_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "socks_handle_contents_rsp: connection(%d : %d) is closed by server.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "socks_handle_contents_rsp: error for read from server. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static void socks_async_connect_server(connection_t *conn)
{
    ...
    if (g_socks_mgr.type == SOCKS_PROXY) {
        if (conn->atype == HOST_IPV4) {
            ...
            {
                LOG_PRINT(LOG_NOTICE, "socks_async_connect_server: error for connect server(%s:%d), connection(%d), error %d",
            ...
            if (phost == NULL) {
                LOG_PRINT(LOG_NOTICE, "socks_async_connect_server: error for convert domain(%s:%d), connection(%d)",
        ...
        if (VIO_SUCCESS != ret)
        {
            LOG_PRINT(LOG_NOTICE, "socks_async_connect_server: error for connect rproxy(%s:%d), connection(%d), error %d",
...
static void socks_handle_conn(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_NOTICE, "socks_handle_conn: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "socks_handle_conn: error for read from client, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    if (conn->buf[0] != 0x05 || conn->buf[1] == 0) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_conn: invalid request of socks, connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (is_need_auth) {
        ...
        if (is_allowed_auth == FALSE) {
            LOG_PRINT(LOG_NOTICE, "socks_handle_conn: invalid authentication, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
            goto err_exit;
        }
        conn->buf[1] = 0x02;
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_conn: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    //
...
static void socks_handle_auth(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_NOTICE, "socks_handle_auth: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "socks_handle_auth: error for read from client, connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    // user
    if (conn->buf[1] == 0) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_auth: invalid request of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    user_len = conn->buf[1];
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_auth: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    // 
...
static void socks_handle_host(connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_NOTICE, "socks_handle_host: connection(%d : %d) is closed by client.",
                conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "socks_handle_host: error for read from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    if (conn->buf[0] != 0x05) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_host: invalid request of socks. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    ...
    if (cmd != 0x01) {
        LOG_PRINT(LOG_NOTICE, "socks_handle_host: UDP is not supported. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    ...
    case HOST_IPV4: // ipv4
    {
        ...
        ret = vio_read(&conn->client_vio, conn->buf, conn->buf_len);
        if (VIO_SUCCESS != ret) {
            LOG_PRINT(LOG_NOTICE, "socks_handle_host: error for read domain from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
            goto err_exit;
        }
        ret = sprintf(conn->host, "%d.%d.%d.%d", conn->buf[4], conn->buf[5], conn->buf[6], conn->buf[7]);
        conn->host[ret] = '\0';
    ...
    case HOST_IPV6:
        ...
        LOG_PRINT(LOG_NOTICE, "socks_handle_host: addr/ipv6 is not supported. connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    conn->atype = atyp;
    ...
    if (VIO_SUCCESS != ret)
    {
        LOG_PRINT(LOG_NOTICE, "socks_handle_host: error for send result of socks, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
    // 
...
static bool32 lproxy_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_req: timeout for client, connection(%d : %d)",
            conn->id, conn->client_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "lproxy_handle_contents_req: connection(%d : %d) is closed by client.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_req: error for read from client. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret)
    {
        LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_req: error for send request to rproxy, connection(%d : %d), error %d",
            conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        goto err_exit;
    }
...
static bool32 lproxy_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "lproxy_handle_contents_rsp: connection(%d : %d) is closed by rproxy.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_rsp: error for read from rproxy. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_rsp: connection(%d : %d) is closed by rproxy.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_rsp: error for read from rproxy. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "lproxy_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static bool32 rproxy_parse_contents_req(connection_t *conn, bool32 *is_first_content)
{
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "rproxy_parse_contents_req: connection(%d : %d) is closed by lproxy.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "rproxy_parse_contents_req: error for read from lproxy. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        }
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_NOTICE, "rproxy_parse_contents_req: connection(%d : %d) is closed by lproxy.",
            conn->id, conn->client_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "rproxy_parse_contents_req: error for read from lproxy. connection(%d : %d), error %d",
                conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
    ...
    *is_first_content = mach_read_from_1((unsigned char*)conn->buf);
    ...
    LOG_PRINT(LOG_DEBUG, "rproxy_parse_contents_req: connection(%d : %d : %d) SERVER(%s : %d)",
...
static bool32 rproxy_handle_contents_req(uint32 events, connection_t *conn)
{
    ...
    else
    {
        ...
        if (VIO_SUCCESS != ret)
        {
            LOG_PRINT(LOG_NOTICE, "rproxy_handle_contents_req: error for send request to server, connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
            goto err_exit;
        }
...
static bool32 rproxy_check_connection_status(uint32 events, connection_t *conn)
{
    ...
    if (VIO_SUCCESS != ret)
    {
        LOG_PRINT(LOG_NOTICE, "rproxy_check_connection_status: error for send request to server, connection(%d : %d), error %d",
            conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        goto err_exit;
    }
...
static bool32 rproxy_handle_contents_rsp(uint32 events, connection_t *conn)
{
    ...
    if (events & EPOLLTIMEOUT) {
        LOG_PRINT(LOG_NOTICE, "rproxy_handle_contents_rsp: timeout for server, connection(%d : %d)",
            conn->id, conn->server_vio.sock_fd);
        goto err_exit;
    }
    ...
    if (VIO_SUCCESS != ret) {
        if (VIO_CLOSE == ret) {
            LOG_PRINT(LOG_DEBUG, "rproxy_handle_contents_rsp: connection(%d : %d) is closed by server.",
                conn->id, conn->server_vio.sock_fd);
        } else {
            LOG_PRINT(LOG_NOTICE, "rproxy_handle_contents_rsp: error for read from server. connection(%d : %d), error %d",
                conn->id, conn->server_vio.sock_fd, conn->server_vio.error_no);
        }
        goto err_exit;
    }
    ...
    ret = vio_write(&conn->client_vio, conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "rproxy_handle_contents_rsp: error for send result to client, connection(%d : %d), error %d",
            conn->id, conn->client_vio.sock_fd, conn->client_vio.error_no);
        goto err_exit;
    }
...
static void socks_epoll_events(int fd, struct epoll_event* event)
{
    ...
    LOG_PRINT(LOG_DEBUG, "socks_epoll_events: connection(%d : %d : %d), fd %d status %d",
...
static bool32 load_config(int argc, char **argv)
{
    ...
    sprintf(logfile, "%s/socks.log", path);
#endif
    log_init(g_socks_mgr.log_level, g_socks_mgr.log_type, logfile);
...
void aes_encrypt_file(char *src_file, char *output_file)
{
    ...
    while (len > 0) {
        if (len % 16 == 0) {
            aes_encrypt_ecb(AES_CYPHER_128, (uint8*)buf, len, encrypt_init_key);
        } else {
            uint32 i, pad_count;
            pad_count = 16 - (len % 16);
        ...
        aes_encrypt_ecb(AES_CYPHER_128, (uint8*)buf, len, encrypt_init_key);
        fwrite(buf, len, 1, w_fp);
        //
        len = fread(buf, 1024 * 1024, 1, r_fp);
    }
...
void aes_decrypt_file(char *src_file, char *output_file)
{
    ...
    while (len > 0) {
        if (len % 16 == 0) {
            aes_decrypt_ecb(AES_CYPHER_128, (uint8*)buf, len, encrypt_init_key);
        } else {
...
void files_encrypt(char * cfg_file)
{
    char        src_file[1024], output_file[1024], item[256];
    int32       type, file_count, len;

    type = get_private_profile_int("general", "type", 0, cfg_file);
    file_count = get_private_profile_int("general", "count", 0, cfg_file);
    while (file_count > 0 && (type == 1 || type == 2)) {
        len = sprintf(item, "src_file_%d", file_count);
        item[len] = '\0';
        get_private_profile_string("general", item, "", src_file, 1023, cfg_file);
        len = sprintf(item, "output_file_%d", file_count);
        item[len] = '\0';
        get_private_profile_string("general", item, "", output_file, 1023, cfg_file);

        if (type == 1) {
            aes_encrypt_file(src_file, output_file);
        } else {
            aes_decrypt_file(src_file, output_file);
        }
        file_count--;
    }
...
{
    files_encrypt("D:\\MyWork\\cos\\etc\\encrypt.ini");
    return;
    ...
    LOG_PRINT(LOG_INFO, "service is started. host %s:%d", g_socks_mgr.local_host, g_socks_mgr.local_port);
