...
int vio_socket_init()
{
    ...
    if (WSAStartup(version, &wd) != 0) {
        LOG_PRINT(LOG_NOTICE, "error for WSAStartup, errno=%d", socket_errno);
        return -1;
    }
#endif
    return 0;
...
int vio_set_blocking(my_socket sock_fd, bool32 non_block, bool32 no_delay)
{
#ifdef __WIN__
    {
        ...
        if (ioctlsocket(sock_fd, FIONBIO, &arg)) {
            LOG_PRINT(LOG_NOTICE, "vio_set_blocking: sock fd %d, error %d", sock_fd, socket_errno);
            return -1;
        }
...
#endif
    {
        LOG_PRINT(LOG_NOTICE, "vio_ip4_to_sockaddr: error for convert %s to sockaddr, error=%d", host, socket_errno);
        return VIO_ERROR;
    }
...
static int vio_connect_poll(Vio* vio, int timeout_ms)
{
    ...
    if (ret == 0) {
        ...
        LOG_PRINT(LOG_NOTICE, "vio_connect_by_addr: timeout for connect");
        goto err_exit;
    }
    if (ret <= 0) {
        vio->error_no = socket_errno;
        LOG_PRINT(LOG_NOTICE, "vio_connect_by_addr: error for vio_socket_poll, error %d", socket_errno);
        goto err_exit;
    }
    if (fds.revents & (POLLERR | POLLHUP)) {
        vio->error_no = socket_errno;
        LOG_PRINT(LOG_NOTICE, "vio_connect_by_addr: can not connect to server, error %d", socket_errno);
        goto err_exit;
    }
...
int vio_connect_async(Vio* vio, const char* host, uint16 port, const char* bind_host)
{
    ...
    if (bind_host != NULL && bind_host[0] != '\0') {
        ...
        if (INVALID_SOCKET == vio->sock_fd) {
            LOG_PRINT(LOG_NOTICE, "vio_connect_async: can not create socket");
            return VIO_ERROR;
        }
        if (bind(vio->sock_fd, (struct sockaddr *)&vio->local, vio->local_len) != 0) {
            vio_close_socket(vio);
            LOG_PRINT(LOG_NOTICE, "vio_connect_async: can not bind ipaddress(%s)", bind_host);
            return VIO_ERROR;
        }
    }
    ...
    if (0 != ret) {
        ...
        if (vio->error_no == SOCKET_EINPROGRESS || vio->error_no == SOCKET_EAGAIN ||
            ...
            vio_close_socket(vio);
            LOG_PRINT(LOG_NOTICE, "vio_connect_async: can not connect to server(%s:%d), error %d",
...
int vio_connect_by_addr_async(Vio* vio, struct sockaddr_in *addr, socklen_t len)
{
    ...
    if (INVALID_SOCKET == vio->sock_fd) {
        LOG_PRINT(LOG_NOTICE, "vio_connect_by_addr_async: can not create socket");
        return VIO_ERROR;
    }
    vio_set_blocking(vio->sock_fd, TRUE, TRUE);
    vio_set_buffer_size(vio->sock_fd, M_TCP_SNDBUF_SIZE, M_TCP_RCVBUF_SIZE);
...
int vio_check_connection_status(Vio* vio)
{
    ...
    if (getsockopt(vio->sock_fd, SOL_SOCKET, SO_ERROR, (char *)&err, (socklen_t*)&err_len) == -1) {
        vio->error_no = socket_errno;
        LOG_PRINT(LOG_NOTICE, "vio_check_connection_status: error for getsockopt, error %d", socket_errno);
        goto err_exit;
    }
    if (err != 0) {//check the SO_ERROR state
        vio->error_no = err;
        LOG_PRINT(LOG_NOTICE, "vio_check_connection_status: SO_ERROR, error %d", err);
        goto err_exit;
    }
...
my_socket vio_socket_listen(  const char* host, uint16 port)
{
    ...
    M_RETURN_IF_ERROR(vio_ip_to_sockaddr(host, port, &domain, (struct sockaddr *)&remote, &remote_len));
    
    sock_fd = vio_create_socket(domain, SOCK_STREAM, 0);
    if (INVALID_SOCKET == sock_fd) {
        LOG_PRINT(LOG_NOTICE, "vio_socket_listen: can not create socket, errno %d", socket_errno);
    ...
    if (listen(sock_fd, backlog) < 0) {
        LOG_PRINT(LOG_NOTICE, "vio_socket_listen: can not listen on host(%s:%d), errno %d", host, port, socket_errno);
...
size_t vio_socket_write(Vio* vio, const char* buf, uint32 size)
{
    size_t ret;
    ...
    while ((ret = send(vio->sock_fd, buf, size, 0)) == -1)
    {
        ...
        if (vio->error_no != SOCKET_EWOULDBLOCK && vio->error_no != SOCKET_EAGAIN && vio->error_no != SOCKET_EINTR) {
            LOG_PRINT(LOG_NOTICE, "vio_socket_write: fd %d errno %d", vio->sock_fd, vio->error_no);
            break;
        }
        ...
        LOG_PRINT(LOG_DEBUG, "vio_socket_read: wait for write, vio fd %d", vio->sock_fd);
...
int vio_write(Vio* vio, const char* buf, uint32 size)
{
    ...
    while (size)
    {
        size_t sentcnt = vio_socket_write(vio, buf, size);
...
status_t vio_write_timed(Vio* vio, const char* buf, uint32 size, uint32 timeout)
{
...
size_t vio_socket_read(Vio* vio, const char* buf, uint32 size)
{
    size_t ret;
    ...
    while ((ret = recv(vio->sock_fd, (SOCKBUF_T *)buf, size, 0)) == -1)
    {
        vio->error_no = socket_errno;
        if (vio->error_no != SOCKET_EWOULDBLOCK && vio->error_no != SOCKET_EAGAIN && vio->error_no != SOCKET_EINTR) {
            LOG_PRINT(LOG_NOTICE, "vio_socket_read: fd %d errno %d", vio->sock_fd, vio->error_no);
            break;
        }
        ...
        LOG_PRINT(LOG_DEBUG, "vio_socket_read: wait for read, vio fd %d", vio->sock_fd);
...
int vio_read(Vio* vio, const char* buf, uint32 buf_size)
{
    ...
    while (buf_size)
    {
        size_t recvcnt = vio_socket_read(vio, buf, buf_size);
        if (recvcnt == VIO_ERROR) {
            if (retry_count < vio->retry_count && vio_should_retry(vio)) {
                retry_count++;
                LOG_PRINT(LOG_NOTICE, "vio_socket_read: retry read, fd %d retry %d", vio->sock_fd, retry_count);
                continue;
            }
            return (VIO_CLOSE == SOCKET_ECONNRESET) ? VIO_CLOSE : VIO_ERROR;;
