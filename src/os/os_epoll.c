...
int epoll_init()
{
    if (g_epfd_pool.epfd_entry_pool == NULL) {
        g_epfd_pool.epfd_entry_pool = biqueue_init(sizeof(epfd_entry_t), 255);
    }
    return 0;
}

static int epoll_ctl_add(epfd_entry_t* epfd_entry, int fd, struct epoll_event* event)
{
    const ib_rbt_node_t  *rbt_node;
    fd_entry_t            fd_entry;

    fd_entry.event = *event;
    fd_entry.fd = (uint32)fd;
    ...
    if (rbt_node == NULL) {
        LOG_PRINT(LOG_NOTICE, "epoll_ctl_add: error for malloc fd_entry, epoll fd %d, fd_entry %d", epfd_entry->epfd, fd);
        return -1;
    }
...
static int epoll_ctl_mod(epfd_entry_t* epfd_entry, int fd, struct epoll_event* event)
{
    const ib_rbt_node_t  *rbt_node;
    ...
    if (fd_entry == NULL) {
        LOG_PRINT(LOG_NOTICE, "epoll_ctl_mod: error for find fd_entry, epoll fd %d, fd_entry %d", epfd_entry->epfd, fd);
        return -1;
    }
...
static int epoll_ctl_del(epfd_entry_t* epfd_entry, int fd, struct epoll_event* event)
{
    bool32                deleted;
    fd_entry_t            key;
    ...
    if (!deleted) {
        LOG_PRINT(LOG_NOTICE, "epoll_ctl_del: error for free fd_entry, epoll fd %d, fd_entry %d", epfd_entry->epfd, fd);
        return -1;
    }
...
int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event)
{
    ...
    if (node == NULL) {
        LOG_PRINT(LOG_NOTICE, "epoll_ctl: error for get epfd_entry, epoll fd %d, fd %d", epfd, fd);
        return -1;
    }
    ...
    switch (op)
    {
        ...
        default:
        {
            LOG_PRINT(LOG_NOTICE, "epoll_ctl: invalid op, epoll fd %d, fd %d op %d", epfd, fd, op);
            return -1;
        }
    }
...
int epoll_create1(int flags)
{
    ...
    if (node == NULL) {
        LOG_PRINT(LOG_NOTICE, "epoll_create1: error for malloc epoll_entry");
        return -1;
    }

    entry = OBJECT_OF_QUEUE_NODE(epfd_entry_t, node, queue_node);
    entry->epfd = node->node_id;
    entry->fd_entry_rbt_lock = 0;
    entry->fd_entry_rbt = rbt_create(sizeof(fd_entry_t), fd_entry_cmp);
                              ...
                              fd_set *rfds, fd_set *wfds, fd_set *efds)
{
    ...
    if (queue_node == NULL) {
        LOG_PRINT(LOG_NOTICE, "epoll_ctl: error for get epfd_entry, epoll fd %d", epfd);
        return -1;
    }
...
int epoll_close(int epfd)
{
    ...
    epfd_entry->fd_entry_rbt_lock = 0;
