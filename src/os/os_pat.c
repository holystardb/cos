...
pat_mgr_t                     g_pat_mgr;
os_thread_key_t(uint8)        g_selfpno_key;
...
uint8 CURR_PNO()
{
    uint8 pno = INVALID_PNO;
    os_thread_get_specific(g_selfpno_key, uint8, pno);
    return pno;
}
...
uint16 CURR_EVENT()
{
    ...
    return g_pat_mgr.pats[pno].event;
}
...
static pat_conn_t* pat_alloc_conn(reactor_t* reactor)
{
    ...
    LOG_PRINT(LOG_DEBUG, "pat_alloc_conn: connection(%d)", conn->id);
...
{
    biqueue_node_t *node;
    LOG_PRINT(LOG_DEBUG, "pat_free_conn: connection(%d)", conn->id);
...
static bool32 pat_epoll_accept(reactor_t* reactor, my_socket listen_fd, my_socket accept_fd)
{
    ...
    if (conn == NULL) {
        LOG_PRINT(LOG_ERROR, "pat_epoll_accept: error for alloc conn, accept fd %d", accept_fd);
        return FALSE;
    }
    ...
    if (-1 == reactor_add_read(conn->reactor, accept_fd, &epoll_data, 0)) {
        LOG_PRINT(LOG_NOTICE, "pat_epoll_accept: connection(id=%d, fd=%d) can not add event to epoll", conn->id, accept_fd);
        pat_free_conn(conn);
        return FALSE;
    }

    LOG_PRINT(LOG_DEBUG, "pat_epoll_accept: connection(id=%d, fd=%d) added event to epoll", conn->id, accept_fd);

    return TRUE;
}

static void pat_epoll_events(int fd, struct epoll_event* event)
{
...
bool32 pat_pool_init(uint8 reactor_count)
{
    ...
    if (!os_is_valid_thread(handle)) {
        LOG_PRINT(LOG_ERROR, "pat_pool_init: Error for create_thread");
        return FALSE;
    }
...
{
    g_pat_mgr.user_len = strlen(user);
    g_pat_mgr.user_len = g_pat_mgr.user_len > 255 ? 255 : g_pat_mgr.user_len;
    strncpy(g_pat_mgr.user, user, g_pat_mgr.user_len);
    g_pat_mgr.user[g_pat_mgr.user_len] = '\0';
    g_pat_mgr.passwd_len = strlen(password);
    g_pat_mgr.passwd_len = g_pat_mgr.passwd_len > 255 ? 255 : g_pat_mgr.passwd_len;
    strncpy(g_pat_mgr.password, password, g_pat_mgr.passwd_len);
...
static void pat_retry_connect(pat_conn_t *conn)
{
    ...
    if (VIO_SUCCESS != vio_connect(&conn->vio, conn->pat->host, conn->pat->port, NULL, TIMEOUT_SECONDS)) {
        LOG_PRINT(LOG_NOTICE, "pat_retry_connect: can not connect to server(pno %d %s:%d), error %d",
            conn->pat->pno, conn->pat->host, conn->pat->port, conn->vio.error_no);
        goto err_exit;
    } else {
        LOG_PRINT(LOG_DEBUG, "pat_retry_connect: connected to server(pno %d %s:%d), connection(%d : %d)",
    ...
    if (VIO_SUCCESS != vio_write(&conn->vio, (const char *)conn->buf, conn->buf_len)) {
        LOG_PRINT(LOG_NOTICE, "pat_retry_connect: error for send auth to server(pno %d %s:%d), connection(%d : %d), error %d",
...
static void* pat_send_thread_entry(void *arg)
{
    ...
    while (pat->type == IS_CLIENT || (pat->type == IS_SERVER && conn->vio.inactive == FALSE))
    {
        ...
        if (pat->pno != data->pno) {
            LOG_PRINT(LOG_NOTICE, "pat_send_thread_entry: Error, dest pno(%d) is invalid, current pat pno(%d)",
                data->pno, pat->pno);
            free(data);
            data = NULL;
            continue;
        ...
        {
            LOG_PRINT(LOG_NOTICE, "pat_send_thread_entry: error for send data to pno(%d), connection(%d : %d), error %d",
                pat->pno, conn->id, conn->vio.sock_fd, conn->vio.error_no);
            pat_close_connection(conn);
        } else {
            LOG_PRINT(LOG_DEBUG, "pat_send_thread_entry: sended data to pno(%d), connection(%d : %d)",
...
static void* pat_worker_thread_entry(void *arg)
{
    ...
    os_thread_set_specific(g_selfpno_key, pat->pno);
...
static void pat_handle_auth_req(pat_conn_t *conn)
{
    ...
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "pat_handle_auth_req: error for read from lproxy, connection(%d : %d), error %d",
    ...
    if (INVALID_PNO == client_pno) {
        LOG_PRINT(LOG_DEBUG, "pat_handle_auth_req: invalid pno, connection(%d : %d)",
    ...
    if (g_pat_mgr.user_len > 0) {
        if (user_len != g_pat_mgr.user_len || passwd_len != g_pat_mgr.passwd_len ||
            ...
            LOG_PRINT(LOG_DEBUG, "pat_handle_auth_req: client pno %d is invalid user or password, connection(%d : %d)",
                client_pno, conn->id, conn->vio.sock_fd);
            goto err_exit;
        } else {
            LOG_PRINT(LOG_DEBUG, "pat_handle_auth_req: client pno(%d) is authenticated by user/password. connection(%d : %d)",
                client_pno, conn->id, conn->vio.sock_fd);
        }
    } else {
        LOG_PRINT(LOG_DEBUG, "pat_handle_auth_req: client pno(%d) is authenticated without user/password. connection(%d : %d)",
            client_pno, conn->id, conn->vio.sock_fd);
    }
    conn->buf[0] = 0;
    conn->buf_len = 1;
    ...
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "pat_handle_auth_req: error for send result of authenticated to client pno %d by connection(%d : %d), error %d",
    ...
    if (!os_is_valid_thread(conn->pat->handle)) {
        LOG_PRINT(LOG_NOTICE, "pat_handle_auth_req: failed to create thread for client pno %d", client_pno);
        goto err_exit;
    }
...
static void pat_handle_auth_rsp(pat_conn_t *conn)
{
    ...
    ret = vio_read(&conn->vio, (const char*)conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_WARN, "handle_auth_rsp: error for read, pno %d connection(%d : %d), error %d",
            conn->pat->pno, conn->id, conn->vio.sock_fd, conn->vio.error_no);
        goto err_exit;
    }
    if (conn->buf[0] != 0) {
        LOG_PRINT(LOG_WARN, "handle_auth_rsp: invalid user or password, pno %d connection(%d : %d)",
            conn->pat->pno, conn->id, conn->vio.sock_fd);
        goto err_exit;
    } else {
        LOG_PRINT(LOG_DEBUG, "handle_auth_rsp: pno %d authenticated, connection(%d : %d)",
            conn->pat->pno, conn->id, conn->vio.sock_fd);
    }
    // 
    conn->status = PAT_CONTENT;
...
static void pat_handle_content(pat_conn_t *conn)
{
    ...
    ret = vio_read(&conn->vio, (const char*)conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "handle_content: error for read lenght of package, pno %d connection(%d : %d), error %d",
            conn->pat->pno, conn->id, conn->vio.sock_fd, conn->vio.error_no);
        goto err_exit;
    }
    ...
    ret = vio_read(&conn->vio, (const char*)conn->buf, conn->buf_len);
    if (VIO_SUCCESS != ret) {
        LOG_PRINT(LOG_NOTICE, "handle_content: error for read data, pno %d connection(%d : %d), error %d",
            conn->pat->pno, conn->id, conn->vio.sock_fd, conn->vio.error_no);
        goto err_exit;
    }
    ...
    if (pat_append_to_list(dest_pno, src_pno, EVENT_RECV, conn->buf + 2, conn->buf_len - 2) == FALSE) {
        LOG_PRINT(LOG_NOTICE, "handle_content: error for append_to_list, dest pno %d src pno %d connection(%d : %d)",
...
static pat_t* pat_init(uint8 pno, pat_callback_func func_ptr)
{
    ...
    if (INVALID_PNO == pno || g_pat_mgr.pats[pno].pno != INVALID_PNO) {
        LOG_PRINT(LOG_NOTICE, "pat_init: Error, pno(%d) is invalid or already exists", pno);
        return NULL;
    }
...
bool32 pat_startup_as_server(uint8 pno, pat_callback_func func_ptr, char *host, uint16 port)
{
    ...
    if (NULL == pat) {
        LOG_PRINT(LOG_NOTICE, "pat_startup_as_server: Error for pat_init, pno %d", pno);
        return FALSE;
    }
    ...
    if (!os_is_valid_thread(pat->handle)) {
        LOG_PRINT(LOG_NOTICE, "pat_startup_as_server: failed to create thread for pno %d", pno);
        goto err_exit;
    }
...
bool32 pat_set_server_info(uint8 pno, char *host, uint16 port)
{
    ...
    if (NULL == pat) {
        LOG_PRINT(LOG_NOTICE, "pat_set_server_info: Error for pat_init, pno %d", pno);
        return FALSE;
    }
    pat->type = IS_CLIENT;
    strcpy(pat->host, host);
    pat->port = port;
...
bool32 pat_startup_as_client(uint8 pno, pat_callback_func func_ptr)
{
    ...
    pat = pat_init(pno, func_ptr);
    if (NULL == pat) {
        LOG_PRINT(LOG_ERROR, "pat_startup_as_client: Error for pat_init, pno %d", pno);
        return FALSE;
    }

    pat->handle = os_thread_create(pat_worker_thread_entry, pat, &pat->thread_id);
    if (!os_is_valid_thread(pat->handle)) {
        LOG_PRINT(LOG_ERROR, "pat_startup_as_client: failed to create thread for pno %d", pno);
        pat_destroy(pat);
        return FALSE;
    }
    ...
    for (i = 0; i < MAX_PAT_COUNT; i++) {
        ...
        if (conn == NULL) {
            LOG_PRINT(LOG_ERROR, "pat_startup_as_client: failed to alloc conn for pno %d", g_pat_mgr.pats[i].pno);
            goto err_exit;
        }
        ...
        if (!os_is_valid_thread(g_pat_mgr.pats[i].handle)) {
            LOG_PRINT(LOG_ERROR, "pat_startup_as_client: failed to create thread for pno %d", g_pat_mgr.pats[i].pno);
            goto err_exit;
        }
    }
