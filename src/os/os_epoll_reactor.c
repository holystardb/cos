...
static void reactor_set_timer(reactor_t *reactor, reactor_data_t *timer, uint32 count100ms)
{
    ...
    LOG_PRINT(LOG_DEBUG, "reactor_set_timer: epoll_fd %d fd=%d", reactor->epoll_fd_with_timeout, timer->fd);
...
static void reactor_del_timer(reactor_t *reactor, reactor_data_t *timer)
{
    ...
    LOG_PRINT(LOG_DEBUG, "reactor_del_timer: epoll_fd %d fd=%d", reactor->epoll_fd_with_timeout, timer->fd);
...
static reactor_data_t* alloc_reactor_data(reactor_t *reactor, int fd)
{
...
static reactor_data_t* get_reactor_data(reactor_t *reactor, int fd)
{
    const ib_rbt_node_t  *rbt_node;
    reactor_data_t        key, *data = NULL;
...
static void free_reactor_data(reactor_t *reactor, int fd)
{
    reactor_data_t key;
    key.fd = fd;
...
static int reactor_add(reactor_t *reactor, int fd, epoll_data_t *data, uint32 events, uint16 timeout_sec)
{
    struct epoll_event  ev;
    reactor_data_t     *r_data;

    LOG_PRINT(LOG_DEBUG, "reactor_add: epoll_fd %d fd %d", reactor->epoll_fd_with_timeout, fd);
...
int reactor_add_read(reactor_t *reactor, int fd, epoll_data_t *data, uint16 timeout_sec)
{
    uint32 events;
...
int reactor_add_write(reactor_t *reactor, int fd, epoll_data_t *data, uint16 timeout_sec)
{
    uint32 events;
...
int reactor_del(reactor_t *reactor, int fd)
{
    ...
    LOG_PRINT(LOG_DEBUG, "reactor_del: epoll_fd %d fd %d", reactor->epoll_fd_with_timeout, fd);
    ...
    if (r_data != NULL) {
        ...
        spin_unlock(&reactor->lock);
        LOG_PRINT(LOG_NOTICE, "reactor_del: epoll_fd %d can not found fd %d", reactor->epoll_fd_with_timeout, fd);
    }
    
    return 0;
}

static int reactor_mod_oneshot(reactor_t *reactor, int fd, epoll_data_t *data, uint32 events, uint16 timeout_sec)
{
    struct epoll_event  ev;
    reactor_data_t     *r_data;

    LOG_PRINT(LOG_DEBUG, "reactor_mod: epoll_fd %d fd %d", reactor->epoll_fd_with_timeout, fd);

    spin_lock(&reactor->lock, NULL);
    r_data = get_reactor_data(reactor, fd);
    spin_unlock(&reactor->lock);
    if (r_data == NULL) {
        LOG_PRINT(LOG_NOTICE, "reactor_mod: epoll_fd %d can not found fd %d", reactor->epoll_fd_with_timeout, fd);
        return -1;
    }
    r_data->data = *data;
...
int reactor_mod_read_oneshot(reactor_t *reactor, int fd, epoll_data_t *data, uint16 timeout_sec)
{
    uint32  events;
    events = EPOLLIN | EPOLLONESHOT;
...
int reactor_mod_write_oneshot(reactor_t *reactor, int fd, epoll_data_t *data, uint16 timeout_sec)
{
    uint32  events;
    events = EPOLLOUT | EPOLLONESHOT;
    return reactor_mod_oneshot(reactor, fd, data, events, timeout_sec);
...
int reactor_epoll_add_read(reactor_t *reactor, int fd, epoll_data_t *data)
{
    struct epoll_event  ev;
...
int reactor_epoll_del(reactor_t *reactor, int fd)
{
...
int reactor_epoll_mod_oneshot(reactor_t *reactor, int fd, epoll_data_t *data)
{
    struct epoll_event  ev;
    ev.events = EPOLLIN | EPOLLONESHOT;
    ev.data = *data;
    ...
    while (!pool->is_end)
    {
        ...
        if (INVALID_SOCKET == accept_fd) {
            ...
            if (error_no != SOCKET_EINTR && error_no != SOCKET_EAGAIN && error_no != SOCKET_EWOULDBLOCK) {
                LOG_PRINT(LOG_WARN, "reactor_handle_accept: error for accept, error %d", error_no);
            }
            return;
        }

        if (vio_getnameinfo(&in_addr, ip_addr, sizeof(ip_addr), NULL, 0, NI_NUMERICHOST)) {
            LOG_PRINT(LOG_WARN, "reactor_handle_accept: fails to print out IP-address, fd %d", accept_fd);
            close_socket(accept_fd);
            continue;
        }
    
        if (-1 == vio_set_blocking(accept_fd, TRUE, TRUE)) {
            LOG_PRINT(LOG_WARN, "reactor_handle_accept: error for vio_set_blocking, fd %d", accept_fd);
            close_socket(accept_fd);
            continue;
        }
        ...
        if (FALSE == pool->acpt_func(reactor, listen_fd, accept_fd)) {
            LOG_PRINT(LOG_WARN, "reactor_handle_accept: socket(fd %d) is closed", accept_fd);
            close_socket(accept_fd);
        }
    }
}
...
{
    if (os_is_valid_thread(pool->acpt_thread)) {
        return FALSE;
    }
...
static void reactor_handle_events_with_timeout(reactor_t *reactor)
{
    ...
    while (r_data != NULL) {
        ...
        LOG_PRINT(LOG_DEBUG, "reactor_handle_events: timeout, reactor epoll_fd %d fd %d",
    ...
    if (nfds == -1) {
        LOG_PRINT(LOG_ERROR, "reactor_handle_events: error for epoll_wait, reactor epoll_fd = %d",
    ...
    for (loop = 0; loop < nfds; ++loop) {
        ...
        if (r_data->timeout_100ms > 0) {
            LOG_PRINT(LOG_DEBUG, "reactor_handle_events: delete timer, reactor epoll_fd %d fd %d",
                reactor->epoll_fd_with_timeout, r_data->fd);
            reactor_del_timer(reactor, r_data);
        }
    }
