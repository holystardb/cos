...
struct	ib_rbt_struct {
	ib_rbt_node_t* nil; /* Black colored node that is
						used as a sentinel. This is pre-allocated too.*/

	ib_rbt_node_t* root; /* Root of the tree, this is
						pre-allocated and the first data node is the left child.*/
...
ib_rbt_node_t* rbt_remove_node(
	...
	const ib_rbt_node_t* node);			/*!< in: node to delete, this
						...
						because the caller has access only to const nodes.*/
...
const ib_rbt_node_t* rbt_insert(
	...
	const void*	value);			/*!< in: data that will be copied to the node.*/
