...
/*
 * The date type is represented by a 64-bit signed integer.
 * The minimum unit is 1 microsecond.
 * This indicates the precision can reach up to 6 digits after the decimal point.
 */
...
#define CT_SET_ERROR_DATETIME_OVERFLOW() \
    CM_THROW_ERROR(ERR_TYPE_DATETIME_OVERFLOW, CM_MIN_YEAR, CM_MAX_YEAR);

#define CT_SET_ERROR_TIMESTAMP_OVERFLOW() \
    CM_THROW_ERROR(ERR_TYPE_TIMESTAMP_OVERFLOW, CM_MIN_YEAR, CM_MAX_YEAR);
...
#define cm_get_num_and_check(part_len, start, end, date_text, num_value)         \
    do {                                                                         \
        uint16 item_len = cm_get_num_len_in_str(date_text, part_len, FALSE);  \
        if (item_len == 0) {                                                     \
            return CM_ERROR;                                                     \
        }                                                                        \
                                                                                 \
...
#define cm_get_num_and_check_with_sign(part_len, start, end, date_text, num_value_with_sign)         \
    do {                                                                                             \
        uint16 item_len = cm_get_num_len_in_str(date_text, part_len, TRUE);                       \
        if (item_len == 0) {                                                                         \
            return CM_ERROR;                                                                         \
        }                                                                                            \
                                                                                                     \
