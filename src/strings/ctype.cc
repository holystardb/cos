...
static size_t scan_one_character(const char *s, const char *e, my_wc_t *wc) {
  ...
  {
    int rc = cs->cset->mb_wc(cs, wc, (const uchar *)(s), (const uchar *)(e));
...
uint my_string_repertoire(const CHARSET_INFO *cs, const char *str,
  ...
  if (cs->mbminlen == 1) {
    ...
    my_wc_t wc;
    int chlen;
    for (; (chlen = cs->cset->mb_wc(cs, &wc, (const uchar *)(str), (const uchar *)(strend))) > 0;
         str += chlen) {
...
static size_t my_convert_internal(char *to, size_t to_length,
  ...
  while (1) {
    if ((cnvres = (*mb_wc)(from_cs, &wc, (const uchar *)(from), from_end)) > 0)
      from += cnvres;
...
size_t my_convert(char *to, size_t to_length, const CHARSET_INFO *to_cs,
  ...
  */
  ...
  length = length2 = ut_min(to_length, from_length);
  ...
  */
  for (; length >= 4; length -= 4, from += 4, to += 4) {
    if (mach_read_from_4((uchar *)from) & 0x80808080) break;
    mach_write_to_4((uchar *)to, mach_read_from_4((uchar *)from));
  }
#endif /* __i386__ */
...
bool my_is_prefixidx_cand(const CHARSET_INFO *cs, const char *wildstr,
  ...
  *prefix_len = 0;
  ...
  while (wildstr < wildend) {
    int res;
    if ((res = cs->cset->mb_wc(cs, &wc, (const uchar *)(wildstr),
