...
static int decimal_bin_size_inline(int precision, int scale);
...
int decimal2string(const decimal_t *from, char *to, int *to_len,
  ...
  len = from->sign + intg_len + ut_test(frac) + frac_len;
  if (fixed_precision) {
  ...
  *to_len = len;
  ...
  if (frac) {
    ...
    *s1++ = '.';
    for (; frac > 0; frac -= DIG_PER_DEC1) {
      dec1 x = *buf++;
      for (i = ut_min(frac, DIG_PER_DEC1); i; i--) {
        dec1 y = x / DIG_MASK;
        *s1++ = '0' + (uchar)y;
        x -= y * DIG_MASK;
        x *= 10;
  ...
  if (intg) {
    ...
    for (buf = buf0 + ROUND_UP(intg); intg > 0; intg -= DIG_PER_DEC1) {
      dec1 x = *--buf;
      for (i = ut_min(intg, DIG_PER_DEC1); i; i--) {
        dec1 y = x / 10;
        *--s = '0' + (uchar)(x - y * 10);
        x = y;
      }
...
int string2decimal(const char *from, decimal_t *to, const char **end) {
  ...
  *end = endp;
  ...
  if (endp + 1 < end_of_string && (*endp == 'e' || *endp == 'E')) {
    int str_error;
    longlong exponent = my_strtoll10(endp + 1, (char **)&end_of_string, &str_error);
...
int decimal2double(const decimal_t *from, double *to) {
  ...
  *to = my_strtod(strbuf, (char **)&end, &error);
...
int decimal2bin(decimal_t *from, uchar *to, int precision, int frac) {
  ...
  if (intg1x) {
    ...
    switch (i) {
      case 1:
        mach_write_to_1(to, x);
        break;
      case 2:
        mach_write_to_2(to, x);
        break;
      case 3:
        mach_write_to_3(to, x);
        break;
      case 4:
        mach_write_to_4(to, x);
        break;
      default:
        DBUG_ASSERT(0);
    }
...
int bin2decimal(const uchar *from, decimal_t *to, int precision, int scale,
  ...
  if (intg0x) {
    ...
    switch (i) {
      case 1:
        x = mach_read_from_1(from);
        break;
      case 2:
        x = mach_read_from_2(from);
        break;
      case 3:
        x = mach_read_from_3(from);
        break;
      case 4:
        x = mach_read_from_4(from);
        break;
      default:
        DBUG_ASSERT(0);
    }
  ...
  for (stop = from + intg0 * sizeof(dec1); from < stop; from += sizeof(dec1)) {
    DBUG_ASSERT(sizeof(dec1) == 4);
    *buf = mach_read_from_4(from) ^ mask;
...
int decimal_round(const decimal_t *from, decimal_t *to, int scale,
  ...
  if (to != from) {
    dec1 *p0 = buf0 + intg0 + ut_max(frac1, frac0);
    dec1 *p1 = buf1 + intg0 + ut_max(frac1, frac0);
    ...
    buf1 = to->buf;
    to->sign = from->sign;
    to->intg = ut_min(intg0, len) * DIG_PER_DEC1;
  }
  ...
  if (*buf1 >= DIG_BASE) {
    ...
    *buf1 -= DIG_BASE;
    ...
    if (unlikely(carry)) {
      ...
      for (buf1 = to->buf + intg0 + ut_max(frac0, 0); buf1 > to->buf; buf1--) {
...
int decimal_result_size(decimal_t *from1, decimal_t *from2, char op,
  ...
  switch (op) {
    case '-':
      return ROUND_UP(ut_max(from1->intg, from2->intg)) +
             ROUND_UP(ut_max(from1->frac, from2->frac));
    case '+':
      return ROUND_UP(ut_max(from1->intg, from2->intg) + 1) +
             ROUND_UP(ut_max(from1->frac, from2->frac));
    case '*':
...
static int do_add(const decimal_t *from1, const decimal_t *from2,
  ...
  int intg1 = ROUND_UP(from1->intg), intg2 = ROUND_UP(from2->intg),
      ...
      frac0 = ut_max(frac1, frac2), intg0 = ut_max(intg1, intg2), error;
  ...
  to->sign = from1->sign;
  to->frac = ut_max(from1->frac, from2->frac);
...
static int do_sub(const decimal_t *from1, const decimal_t *from2,
  ...
  int intg1 = ROUND_UP(from1->intg), intg2 = ROUND_UP(from2->intg),
      frac1 = ROUND_UP(from1->frac), frac2 = ROUND_UP(from2->frac);
  int frac0 = ut_max(frac1, frac2), error;
...
static int do_div_mod(const decimal_t *from1, const decimal_t *from2,
  ...
  if (!(tmp1 = (dec1 *)alloca(len1 * sizeof(dec1)))) return E_DEC_OOM;
  ...
  */
  ...
  if (mod) {
    ...
    if (intg0 <= 0) {
      ...
      stop1 = start1 + frac0 + intg0;
      to->intg = ut_min(intg0 * DIG_PER_DEC1, from2->intg);
    }
