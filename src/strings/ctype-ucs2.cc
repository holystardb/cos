...
bs:
  ...
  return negative ? -(longlong)res : res;
}
...
static double my_strntod_mb2_or_mb4(const CHARSET_INFO *cs, char *nptr,
  ...
  *endptr = b;
  res = my_strtod(buf, (char **)endptr, err);
...
static ulonglong my_strntoull10rnd_mb2_or_mb4(const CHARSET_INFO *cs,
  ...
  res = my_strntoull10rnd_8bit(cs, buf, b - buf, unsign_fl, (char **)endptr, err);
  *endptr = (char *)(nptr + cs->mbminlen * (size_t)(*endptr - buf));
  return res;
}
}  // extern "C"
...
static int my_utf16le_uni(const CHARSET_INFO *cs MY_ATTRIBUTE((unused)),
  ...
  if ((*pwc = mach_read_from_2(s)) < MY_UTF16_SURROGATE_HIGH_FIRST ||
  ...
  if ((lo = mach_read_from_2(s)) < MY_UTF16_SURROGATE_LOW_FIRST ||
...
static int my_uni_utf16le(const CHARSET_INFO *cs MY_ATTRIBUTE((unused)),
  ...
  wc -= 0x10000;
  mach_write_to_2(s, (0xD800 | ((wc >> 10) & 0x3FF)));
  s += 2;
  mach_write_to_2(s, (0xDC00 | (wc & 0x3FF)));
...
static size_t my_lengthsp_utf16le(const CHARSET_INFO *cs MY_ATTRIBUTE((unused)),
                                  const char *ptr, size_t length) {
  const char *end = ptr + length;
  while (end > ptr + 1 && mach_read_from_2((const uchar *)end - 2) == 0x20) end -= 2;
  return (size_t)(end - ptr);
}
}  // extern "C"
...
static int my_strnncollsp_utf32_bin(
  ...
  for (minlen = ut_min(slen, tlen); minlen; minlen -= 4) {
...
static size_t my_scan_utf32(const CHARSET_INFO *cs, const char *str,
  ...
  switch (sequence_type) {
    case MY_SEQ_SPACES:
      for (; str < end;) {
        my_wc_t wc;
        int res = my_utf32_uni(cs, &wc, (const uchar *)(str),
                               (const uchar *)(end));
        if (res < 0 || wc != ' ') break;
        str += res;
      }
...
static int my_strnncollsp_ucs2(const CHARSET_INFO *cs, const uchar *s,
  ...
  for (minlen = ut_min(slen, tlen); minlen; minlen -= 2) {
