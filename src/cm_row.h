...
#define ROW_SPRS_COLUMNS        (uint32)1024
#define ROW_MAX_COLUMNS         (uint32)4096
#define ROW_MAX_SIZE            (uint32)8000



#define ROW_INVALID_ITL_ID      UINT_MAX8
...
#define REC_NULL_BITS_IN_BYTES(b)   (((b) + 7) / 8)

typedef struct st_rec_header {
    union {
        struct {
            uint16 size;               // row size, must be the first member variable in row_head_t
            uint16 column_count : 10;  // column count
    ...
    uint8 itl_id;                       // row itl_id
    uint8 null_bits[FLEXIBLE_ARRAY_MEMBER];  // bitmap is no used for CSF

} rec_header_t;  // following is bitmap of column
...
#define CM_CHECK_ROW_FREE(row, len)                                  \
    {                                                                \
        if ((row)->head->size + (len) > (row)->max_size) {           \
            CM_THROW_ERROR(ERR_TOO_BIG_RECORD, (row)->max_size); \
            return CM_ERROR;                                         \
        }                                                            \
    }

#define IS_SPRS_ROW(row) (UNLIKELY((row)->column_count == 0))
...
{
    if (IS_SPRS_ROW(row)) {
        CM_ASSERT(row->sprs_count >= ROW_SPRS_COLUMNS);
        return (uint16)(ut_align16(row->sprs_count - 4) / 4);
    } else {
        return (uint16)((row->column_count <= 12) ? 0 : (ut_align16(row->column_count - 12) / 4));
    }
}

static inline uint16 col_bitmap_ex_size(uint16 cols)
{
    if (cols >= ROW_SPRS_COLUMNS) {
        return (uint16)(ut_align16(cols - 4) / 4);
    } else {
        return (uint16)((cols <= 12) ? 0 : (ut_align16(cols - 12) / 4));
    }
}
...
{
    if (cols >= ROW_SPRS_COLUMNS) {
        return (uint16)(ut_align16(cols - 4) / 4) + 1;
    } else {
        return (uint16)((cols <= 12) ? 0 : (ut_align16(cols - 12) / 4)) + 3;
    }
}
...
{
    if (column_count >= ROW_SPRS_COLUMNS) {
...
static inline status_t csf_put_lob(row_assist_t *ra, uint32 lob_locator_size, var_lob_t *lob)
{
    ...
    switch (lob->type) {
        ...
        case CT_LOB_FROM_VMPOOL:
            ...
            break;
        default:
            CM_THROW_ERROR(ERR_UNKNOWN_LOB_TYPE, "do put csf");
            return CM_ERROR;
    }
...
THREAD_LOCAL uint8 g_row_offset[2];
...
{
    if (ra->is_csf) {
        ra->head->size = ut_align4(ra->head->size);
    }
}
...
static inline uint16 row_column_bits_size(uint16 col_num)
{
    ...
    return (uint16)(ut_align16(col_num) >> 2);
}
    ...
    *(uint16 *)addr = (uint16)lob_locator_size;
    ...
    switch (lob->type) {
        ...
        case CT_LOB_FROM_VMPOOL:
            ...
            break;
        default:
            CM_THROW_ERROR(ERR_UNKNOWN_LOB_TYPE, "do row put");
            return CM_ERROR;
    }
